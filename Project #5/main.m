...
% An estimate of a quantity is biased if its expected value is not equal 
%       to the quantity it estimates.

% xcorr provides the unbiased estimate, dividing by N-|m|, when you specify
%       an 'unbiased' flag after the input sequences.
% The end points (near -(N-1) and N-1) suffer from large variance because
%       xcorr computes them using only a few data points.
% Bias is more desireable because it avoids random large variations at the 
%       end points of the correlation sequence.
...
% The results changed due to the change of window (triang) and it minimized
% variation on the size while keeping the peak relatively the same. 
...
for p = 2:7
   ...
   totsum = 0;
   for k = 1:p
       totsum = totsum + (lev(k))*exp(1j*w_k*k);
   end
   H = 1./(1+totsum);
   phi = abs(H).^2;
   if p ==3
